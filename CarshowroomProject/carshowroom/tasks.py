...
from .models import CarshowroomModel, CarshowroomCar, SupplierPurchaseHistory, CarshowroomDiscountModel
from core.models import CarModel
from suppliers.models import SupplierCar
from suppliers.models import DiscountModel
from customers.models import PurchaseHistory, CustomerModel

def number_of_car_purchases(car, carshowroom):
    return PurchaseHistory.objects.filter(car = car, carshowroom = carshowroom).count()
...
def get_individual_discount(carshowroom, customer):
    number_of_purchases = PurchaseHistory.objects.filter(customer=customer, carshowroom=carshowroom).count()
...
def get_individual_supplier_discount(carshowroom, supplier):
    return SupplierPurchaseHistory.objects.filter(
...
def accept_supplier():
    ...
    for carshowroom in carshowroom_set:
        cars = CarModel.objects.filter(**carshowroom.car_characteristic)
        for car in cars:
            # индивидуальная скидка supplier не учитывается
            supplier_car = SupplierCar.objects.filter(car=car).order_by(
               (1 - max_percent_discount(F('supplier'), car) / 100) * F('price')
            ).first()
            if supplier_car:
...
def buy_suppliers_cars():
    ...
    for carshowroom_car in carshowroom_car_set:
        ...
        if supplier_car:
            ...
            discount_price = supplier_car.price * (1 - (max_percent_discount(supplier, car) + individual_discount)/100)
...
@shared_task
def accept_offer(user_id, json_characteristics):
    car_set = CarModel.objects.filter(**json_characteristics)
    customer = CustomerModel.objects.get(user_id = user_id)
    carshowroom_car_set = CarshowroomCar.objects.filter(car__in = car_set, number__gt = 0).order_by(
            (1 - max_percent_discount(F('carshowroom'), F('car'))/100 * F('price'))
    )
    ...
    if carshowroom_car_set.exists():
        ...
        price = carshowroom_car.price * (1 - (max_percent_discount(carshowroom=carshowroom, car=carshowroom_car.car) + individual_discount)/100)
